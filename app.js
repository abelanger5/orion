...
ipc.on("connect", function(event, data) {
    ...
    io.on('connection', function(socket) {
        ...
        ipc.on("start-app", function(event, data) {
            if (data.id == 3 && !isChrome) {
                isChrome = true; 

                chromeLauncher.launch({
                    startingUrl: 'https://netflix.com/login',
                ...
                for (var i = 0; i < 4; i++) {
                    if (players[i] != null) {
                        ...
                        sockets[i].emit("start-app", {id: 2, name: "game", content: null, id: (i - num_null)}); 
                    } else {
                        num_null++; 
                    }
                }
                ...
                socket.emit("start-app", {id: 1, name: "youtube", content: null}); 
                active = 1; 
            }
        }); 
